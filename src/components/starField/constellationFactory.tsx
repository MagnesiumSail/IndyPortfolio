...
function kSeedBuckets(points: THREE.Vector3[], k: number, rng = defaultRng) {
  ...
  while (seeds.length < k && seeds.length < n) {
    ...
    for (let i = 0; i < n; i++) {
      let dmin = Infinity;
      for (const s of seeds) dmin = Math.min(dmin, points[i].distanceTo(points[s]));
      if (dmin > bestDist) { bestDist = dmin; best = i; }
    }
...
export function buildConstellations(stars: StarData[], opts: ConstellationOpts = {}): Edge[] {
  const { topFraction = 0.07, groups = 3, rng = defaultRng } = opts;
  if (stars.length === 0) return [];

  // pick top by scale
  const k = Math.max(3, Math.floor(stars.length * topFraction));
  const sortedIdx = [...stars.keys()].sort((a, b) => stars[b].scale - stars[a].scale);
  const topIdx = sortedIdx.slice(0, k);
  ...
  const bucketsLocal = kSeedBuckets(topPts, Math.min(groups, topIdx.length), rng);
